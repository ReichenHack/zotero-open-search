Zotero.OpenSearch = {
	...
	init: function () {
		// Connect to (and create, if necessary) zotero-open-search.sqlite in the Zotero directory
		this.DB = new Zotero.DBConnection('zotero-open-search');
	...
	notifierCallback: {
		notify: function(event, type, ids, extraData) {
			if (event == 'add' || event == 'modify' || event == 'delete') {
				...
				Zotero.OpenSearch.DB.query("UPDATE changes SET num = num + 1");
				...
				var str = document.getElementById('zotero-open-search-strings').
...
window.addEventListener('load', function(e) { Zotero.OpenSearch.init(); }, false);
